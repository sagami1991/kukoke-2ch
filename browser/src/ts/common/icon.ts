...
export type TIconSize = "s" | "m";

const fixedSizeMap: {[key in TIconName]?: {width: number, height: number}} = {
	"icon-ok": { width: 30, height: 20}
};
export function getSvgIcon(icon: TIconName, size?: TIconSize, className?: string): string {
